...
def test_examples(value, parts, expected):
...
def test_properties(value, parts):
...
def test_matches_expected_distribution(value, parts):
    base = value // parts
    rem = value % parts
    expected = [base] * (parts - rem) + [base + 1] * rem
    assert split_integer_module.split_integer(value, parts) == expected

def test_when_value_less_than_parts_adds_zeros():
